...
def test_forge_build_matches_bare_request(ioc):
    """The natural, unprefixed phrasing that used to fall through to no
    match at all — Forge had zero front-door presence in the catalog
    despite being the organ that actually generates and tests code."""
    r = ioc.op_interpret('a script that prints "hello world"')
    assert r["matched"] is True
    assert r["intent"] == "forge_build"
    assert r["organ"] == "forge"
    assert r["method"] == "POST"
    assert r["path"] == "/forge/build"
    assert r["body"]["language"] == "python"
    assert r["body"]["filename"] == "main.py"
    assert "prints" in r["body"]["spec"]


def test_forge_build_matches_verb_prefixed_phrasing(ioc):
    r = ioc.op_interpret("write me a function to calculate the factorial of a number")
    assert r["intent"] == "forge_build"
    assert "factorial" in r["body"]["spec"]


def test_forge_build_honors_explicit_filename(ioc):
    r = ioc.op_interpret("build a script called deploy.sh that lists running containers")
    assert r["intent"] == "forge_build"
    assert r["body"]["filename"] == "deploy.sh"


def test_forge_build_detects_node_language_with_intervening_word(ioc):
    """'a node script' has a language descriptor between the article and
    the noun ('script') — must still match, and must map to Sandbox's
    actual language key ('node'), not the input word ('javascript')."""
    r = ioc.op_interpret("a node script that returns the sum of two numbers")
    assert r["intent"] == "forge_build"
    assert r["body"]["language"] == "node"
    assert r["body"]["filename"] == "main.js"

    r = ioc.op_interpret("a javascript function that reverses a string")
    assert r["body"]["language"] == "node"


def test_forge_build_does_not_shadow_orchestrator_or_reflection_intents(ioc):
    """The new pattern sits at the end of the catalog and must not
    accidentally intercept unrelated phrasing that happens to contain
    similar words."""
    assert ioc.op_interpret("restart ollama")["intent"] == "orchestrator_restart"
    assert ioc.op_interpret("start reflecting")["intent"] == "reflection_enable"
    assert ioc.op_interpret("stop reflecting")["intent"] == "reflection_disable"
    assert ioc.op_interpret("is ollama running")["intent"] == "orchestrator_status"
